 ...
 * @author Lolmewn
 */
public class StatsSetCommand extends SubCommand {
    
    private final Main plugin;

    public StatsSetCommand(Main plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(Dispatcher sender, String[] args) {
        if(args.length < 2){
            ...
            sender.sendMessage("/stats set [player] <stat> <value>");
            sender.sendMessage("/stats set <stat> <value> (if [player] is you)");
            return;
        }
        UUID targetPlayer;
        int offset = -1;
        if(args.length == 2){
            ...
            if(!sender.isPlayer()){
                ...
                sender.sendMessage("Try /stats set <player> <stat> <value> instead");
                return;
            }
        ...
        try{
            ...
            holder.removeStat(stat);
            holder.addEntry(stat, new DefaultStatEntry(value));
            sender.sendMessage("Set " + stat.getName() + " to value " + value);
